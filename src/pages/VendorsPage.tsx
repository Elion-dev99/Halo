...
import type { Vendor, VendorInput } from "@/types/arAp";
...
export function VendorsPage() {
  ...
  async function reload() {
    ...
    setError(null);
    try {
      setRows(await listVendors(orgId));
    } catch (err) {
      console.error(err);
      setError("仕入先の読み込みに失敗しました。");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function onSubmit(event: FormEvent) {
    ...
    setError(null);
    try {
      if (editingId) await updateVendor(orgId, editingId, form);
      else await createVendor(orgId, form);
      setForm(empty());
      setEditingId(null);
      await reload();
    } catch (err) {
  ...
  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h2>仕入先</h2>
          <p className="muted">買掛（AP）の取引先マスタです。</p>
        </div>
      </div>

      {canWrite ? (
        <form className="panel" onSubmit={onSubmit}>
          <h3>{editingId ? "仕入先を編集" : "仕入先を追加"}</h3>
